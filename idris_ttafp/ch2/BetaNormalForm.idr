...
data InBNF : Term -> Type where
    ...
    BNFLambda : InBNF lambdaBody -> InBNF (Lambda type lambdaBody)
...
lambdaBodyBNF : InBNF (Lambda type lambdaBody) -> InBNF lambdaBody
lambdaBodyBNF (BNFLambda body_bnf) = body_bnf

lambdaAppNotBNF : Not (InBNF (App (Lambda type lambdaBody) right))
...
isInBNF (App (Lambda type lambdaBody) right) = No lambdaAppNotBNF
isInBNF (Lambda type lambdaBody) =
